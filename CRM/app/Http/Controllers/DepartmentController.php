...
//        print_r($crm_dep_first);exit;
    ...
    //是否启用
    public function epartment_is( Request $request ){
        $data = $request -> post();
//        print_r($data);exit;
//        $is = '';
...
//        dd($crm_dep_get);exit;
        ...
        foreach( $crm_dep_get as $k => $v ){
            if( $v->status == 1){
                $arrs = '<span class="layui-btn layui-btn-sm layui-btn-radius layui-btn-normal is" onclick="student_is( '.$v->d_id.' , '.$v->status.' )" >已启用</span>';
//            return[ 'msg'=> '已启用' , 'code' => '1' , 'data' => $data , 'ids' => $data['ids']];
            }
            if( $v->status == 0){
                $arrs = '<span class="layui-btn layui-btn-sm layui-btn-radius layui-btn-primary is" onclick="student_is( '.$v->d_id.' , '.$v->status.' )" >未启用</span>';
//            return[ 'msg'=> '已禁用' , 'code' => '1' , 'data' => $data  , 'ids' => $data['ids'] ];
            }
            $arr .= '<tr>
                        <td>
