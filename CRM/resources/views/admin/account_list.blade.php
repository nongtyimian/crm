                ...
                <td class="td-status">
                    ...
                    @if( $v->status === 0 )
                        <span class="layui-btn layui-btn-sm layui-btn-radius layui-btn-primary is"
                              onclick="student_is( {{$v->admin_id}} , {{$v->status}} )">
                                未通过审核
                            </span>
                    @endif
                    @if( $v->status === 1 )
    ...
    function student_is( ids , status ){
        ...
        $.ajax({
            ...
            success: function (datas) {
                ...
                if (datas.code == 1) {
                    if( datas.is == 1 ){
                        layer.msg("已通过审核", {icon: datas.code, time: 1500}, function () {
                            $('#table').html(datas.data);
                        });
                    }
                    if( datas.is == 0 ){
                        layer.msg("未通过审核", {icon: datas.code, time: 1500}, function () {
                            $('#table').html(datas.data);
                        });
                    }
